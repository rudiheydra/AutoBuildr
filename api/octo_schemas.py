...
# Valid dependency types (Feature #209)
VALID_DEPENDENCY_TYPES = frozenset([
    "fixture", "mock", "service", "database", "file", "environment", "api", "component"
])
...
# =============================================================================
# TestContract JSON Schema Definition (Feature #209)
# =============================================================================
...
# Feature #209: Schema for dependencies (fixtures, mocks needed)
TEST_DEPENDENCY_SCHEMA: dict[str, Any] = {
    "$schema": "https://json-schema.org/draft/2020-12/schema",
    "$id": "https://autobuildr.dev/schemas/test-dependency.json",
    "title": "TestDependency",
    "description": "Schema for a test dependency (fixture, mock, service) within a TestContract",
    "type": "object",
    "required": ["name", "dependency_type"],
    "properties": {
        "name": {
            "type": "string",
            "minLength": 1,
            "maxLength": 100,
            "description": "Unique identifier for the dependency (e.g., 'user_fixture', 'api_mock')"
        },
        "dependency_type": {
            "type": "string",
            "enum": list(VALID_DEPENDENCY_TYPES),
            "description": "Type of dependency (fixture, mock, service, database, etc.)"
        },
        "description": {
            "type": "string",
            "maxLength": 500,
        ...
        "setup_hints": {
            ...
            "items": {"type": "string"},
            "maxItems": 10,
            "description": "Optional hints for how to set up the dependency"
        },
        "required": {
            "type": "boolean",
            "default": True,
...
TEST_CONTRACT_SCHEMA: dict[str, Any] = {
    ...
    "title": "TestContract",
    "description": "Schema for TestContract objects generated by Octo (Feature #209)",
    "type": "object",
    ...
    "properties": {
        ...
        # Feature #209: test_name field
        "test_name": {
            "type": "string",
            "maxLength": 200,
            "description": "Human-readable name for this test contract"
        },
        "agent_name": {
            "type": "string",
            "minLength": 1,
        ...
        "test_type": {
            ...
            "description": "Type of testing (unit, integration, e2e, etc.)"
        },
        # Feature #209: subject field
        "subject": {
            "type": "string",
            "maxLength": 500,
            "description": "What is being tested (component, function, feature, endpoint)"
        },
        "assertions": {
            "type": "array",
            "items": TEST_CONTRACT_ASSERTION_SCHEMA,
            "description": "List of assertions that describe expected behaviors"
        },
        "pass_criteria": {
            "type": "array",
            "items": {"type": "string"},
        ...
        # Feature #209: dependencies field
        "dependencies": {
            "type": "array",
            "items": TEST_DEPENDENCY_SCHEMA,
            "maxItems": 20,
            "description": "List of fixtures, mocks, and other dependencies needed"
        },
        "description": {
            "type": "string",
...
def validate_test_contract_schema(data: dict[str, Any]) -> SchemaValidationResult:
    ...
    # Feature #209: Validate test_name (optional but validated if present)
    ...
    # Feature #209: Validate subject (optional but validated if present)
    ...
    # Feature #209: Validate dependencies array
    ...
    if dependencies is not None:
        if not isinstance(dependencies, list):
            ...
            errors.append(SchemaValidationError(
                path="dependencies",
                message="Field 'dependencies' must have at most 20 items",
                code="max_items",
                value=len(dependencies),
            ))
        else:
            for i, dep in enumerate(dependencies):
                _validate_dependency(dep, f"dependencies[{i}]", errors)
...
def _validate_dependency(
    ...
    """
    Validate a single TestDependency.

    Feature #209: Validate dependency structure including name, type, and setup hints.
    """
    if not isinstance(dependency, dict):
        errors.append(SchemaValidationError(
            ...
            code="invalid_type",
            value=type(dependency).__name__,
        ))
        return
    ...
    _validate_string_field(
        ...
        "dependency_type",
        f"{path}.dependency_type",
        errors,
        enum_values=VALID_DEPENDENCY_TYPES,
    )
    ...
    # Optional: setup_hints
    setup_hints = dependency.get("setup_hints")
    if setup_hints is not None:
        if not isinstance(setup_hints, list):
            errors.append(SchemaValidationError(
                path=f"{path}.setup_hints",
                message="Field 'setup_hints' must be an array",
                code="invalid_type",
                value=type(setup_hints).__name__,
            ))
        elif len(setup_hints) > 10:
            errors.append(SchemaValidationError(
                path=f"{path}.setup_hints",
                message="Field 'setup_hints' must have at most 10 items",
                code="max_items",
                value=len(setup_hints),
            ))
        else:
            for i, hint in enumerate(setup_hints):
                if not isinstance(hint, str):
                    errors.append(SchemaValidationError(
                        path=f"{path}.setup_hints[{i}]",
                        message=f"Setup hint at index {i} must be a string",
                        code="invalid_item_type",

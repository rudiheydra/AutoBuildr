...
class Maestro:
    ...
    def _record_agent_planned_event(
        ...
        *,
        ...
        Feature #221: agent_planned audit event type with project/feature linking

        This event is recorded BEFORE Octo invocation to capture the planning
        ...
        try:
            ...
            # Feature #221: Link event to project or feature triggering planning
            if project_name:
                payload["project_name"] = project_name
            if feature_id is not None:
                payload["feature_id"] = feature_id
            # Also use source_feature_id from spec if available and not overridden
            if feature_id is None and spec.source_feature_id is not None:
                payload["feature_id"] = spec.source_feature_id
            ...
            _logger.debug(
                "Recorded agent_planned event: run=%s, spec=%s, event=%d, project=%s, feature=%s",
                run_id,
                spec.name,
                event_id,
                project_name,
                feature_id or spec.source_feature_id,
            )
    ...
    def record_agent_planned(
        ...
        *,
        ...
        This event should be recorded BEFORE Octo invocation to capture the
        planning decision in the audit trail.
